...
async function displayTeam(id) {
    ...
    loadPage('team').then(function () {
        ...
        function checkFav(id, event = false) {
            cekFav(id).then(v => {
                if (v) {
                    teamFavButton.style.backgroundColor = '#0f3460'
                    if (event) {
                        M.toast({html: data.name+' dihapus dari daftar favorit'})
                        hapusTeamFav(id);
                        teamFavButton.style.backgroundColor = '#0f3460'
                    ...
                    if (event) {
                        M.toast({html: data.name + ' ditambahkan ke daftar favorit'})
                        tambahTeamFav(dataFav);
                        teamFavButton.style.backgroundColor = '#e94560'
                        teamFavButton.innerHTML = 'batalkan favorit'
                    }
                }
            })
        }
...
async function displayMatch(id = null) {
    ...
    data.matches.forEach(v => {
        ...
        <div class="card horizontal d-flex f-width-100 align-item-center blue lighten-5">
    ...
    loadPage('fav-team').then(function () {
        ...
        document.querySelectorAll('.unfav').forEach( btn => {
            btn.addEventListener('click',click => {
                click.preventDefault()
                hapusTeamFav(parseInt(click.target.getAttribute('data-id')))
